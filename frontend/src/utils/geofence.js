/**
 * Frontend Geofence Engine (Circle & Polygon)
 * Shared frontend geofence utility guaranteeing 100% agreement with backend logic.
 */
...
export class GeofenceEngine {
  ...
  static formatDistance(meters) {
    if (meters === Infinity || isNaN(meters)) return 'N/A';
  ...
  static distanceToSegment(pLat, pLng, aLat, aLng, bLat, bLng) {
    ...
    if (dAB2 === 0) return this.calculateDistanceMeters(pLat, pLng, aLat, aLng);
  ...
  /**
   * Shortest Distance from Point to Nearest Polygon Edge
   */
  ...
  /**
   * Evaluate Zone State for a Tourist Location
